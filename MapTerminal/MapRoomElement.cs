namespace cs.MapTerminal
{
    ...
    public class MapRoomElement : MapElement
    {
        ...
        public MapRoomElement(int x, int y, int height, int width, string name = "") : base(x, y, height, width)
        {
            Name = name;
        }
        ...
        public override void InsertIntoBuffer(char[,] buffer)
        {
            ...
            new MapBoxElement(this.X, this.Y, this.Height, this.Width).InsertIntoBuffer(buffer);

            // Inserts the text
            // Calculate text offset
            int xOffset = this.Width / 2 - this.Name.Length / 2;
            int yOffset = this.Height / 2;
            new MapTextElement(this.X + xOffset, this.Y + yOffset, Name).InsertIntoBuffer(buffer);
